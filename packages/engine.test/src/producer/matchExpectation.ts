export const matchExpectation = (utils) => {
    return (mockCalls, verbExpectations) => {
        ...
        if(isLonger) return {
            message: () => (`expected calls ${utils.printReceived(mockCalls)} to be same length as expectation ${utils.printExpected(verbExpectations)}`),
            pass: false
        }
        ...
        for(let i = 0; i < mockCalls.length; i++) {
            try {
                ...
                return {
                    message: () => (`expected ${utils.printReceived(mockCalls[i][0])} to equal ${utils.printExpected(verbExpectations[i])}`),
                    pass: false
                }
            }
        }
