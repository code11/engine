...
export = async ({ target, template }: { target: any; template: any }) => {
  const packageName = getTemplatePackageName(template);
  logStep(
    `Instantiating template ${chalk.yellowBright(
      packageName
    )} in the ${chalk.yellowBright(target)} directory`
  );
  ...
  if (!existsSync(templatePackageDir)) {
    logWarning(
      `Template package ${chalk.cyanBright(
        packageName
      )} not found at location ${chalk.cyanBright(templatePackageDir)}`
    );
    process.exit(1);
  }
